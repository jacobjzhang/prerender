...
server.init = function (options) {
  ...
  // Use browserless adapter if BROWSER_WS_ENDPOINT is set, otherwise use local Chrome
  if (this.options.browserWsEndpoint || process.env.BROWSER_WS_ENDPOINT) {
    this.browser = require('./browsers/browserless');
  } else {
